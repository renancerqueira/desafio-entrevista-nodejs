...
export class CompanyFakeBuilder<TBuild = any> {
  private _id: PropOrFactory<string> = () => this.faker.datatype.uuid();
  ...
  private _email_verified: PropOrFactory<string> = () => false;
  ...
  static aCompany() {
    return new CompanyFakeBuilder<Company>();
  }

  static theCompanies(countObjs: number) {
    return new CompanyFakeBuilder<Company[]>(countObjs);
  }

  withUniqueEntityId(valueOrFactory: PropOrFactory<string>) {
  ...
  build(): TBuild {
    const array = new Array(this.countObjs).fill(undefined).map((_, index) => ({
      ...
      email_verified: this.callFactory(this._email_verified, index),
